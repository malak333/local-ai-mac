...
HARNESS_MAX_STEPS="${HARNESS_MAX_STEPS:-96}"
HARNESS_MAX_ABSOLUTE_STEPS="${HARNESS_MAX_ABSOLUTE_STEPS:-256}"
HARNESS_AUTO_EXTEND="${HARNESS_AUTO_EXTEND:-true}"
HARNESS_STEP_EXTENSION="${HARNESS_STEP_EXTENSION:-32}"
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
    ...
    echo "  HARNESS_MAX_STEPS              Soft step limit (default: 96)"
    echo "  HARNESS_MAX_ABSOLUTE_STEPS     Hard step ceiling (default: 256)"
